<div>
    <x-slot:sidebar drawer="main-drawer" class="bg-base-100 lg:bg-inherit">
        ...
        <x-mary-menu activate-by-route>
            <img src="{{ asset('images/logo_azul_e_cinza.png') }}" alt="" class="w-12">
            {{-- User --}}
            @if($user = auth()->user())
                ...
                <x-mary-list-item :item="$user" value="name" sub-value="email" no-separator no-hover class="-mx-2 !-my-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800" link="{{route('profile')}}">
                    <x-slot:actions>
                        {{-- <x-mary-form wire:submit="save">                         
                            <x-slot:actions>
                                <x-button label="Click me!" class="btn-primary" type="submit" spinner="save" icon="o-power"/>
                            </x-slot:actions>
                        </x-mary-form> --}}
            ...
            <x-mary-menu-item title="Armagens" icon="o-globe-alt" link="{{ route('warehouses.index') }}" exact />
            <x-mary-menu-item title="Pontos de coleta" icon="o-globe-alt" link="{{ route('collection-point.index') }}" exact />
            <x-mary-menu-item title="Garagens" icon="o-envelope" link="{{ route('garages.index') }}" exact />
            <x-mary-menu-item title="Carros" icon="o-envelope" link="{{ route('cars.index') }}" exact />
            <x-mary-menu-item title="Lojas" icon="o-x-circle" link="{{ route('stores.index') }}" exact/>
            {{-- Theme toggle --}}
